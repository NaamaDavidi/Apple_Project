...
from Project_Apple.Pages.AppleGlobals import item1, item2, item3, ITEM_TYPE
...
class ExplorePage():
    ...
    def selectItemMacbook(self):
        clickItem = self.driver.find_elements(By.PARTIAL_LINK_TEXT,item1)
    ...
    def selectItemAirpods(self):
        clickItem = self.driver.find_elements(By.PARTIAL_LINK_TEXT,item2)
    ...
    def selectItemIphone(self):
        clickItem = self.driver.find_elements(By.PARTIAL_LINK_TEXT,item3)
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(clickItem[0]))
        clickItem[0].click()
        time.sleep(5)
